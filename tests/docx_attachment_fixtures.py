...
def compound_file(stream_name, payload):
    ...
    for index, (name, kind, first, size, child) in enumerate(
        ...
        struct.pack_into("<HBBIII", directory, start + 64, len(label), kind, 1, 0xFFFFFFFF, 0xFFFFFFFF, child)
    ...
    return bytes(header + directory) + payload.ljust(count * 512, b"\0") + struct.pack("<128I", *fat)


def native_package(name, payload):
    header = struct.pack("<H", 2) + name.encode() + b"\0ignored-path\0" + b"\0" * 8 + b"ignored-temp\0"
