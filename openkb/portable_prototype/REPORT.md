...
**Debian 13.6 GNOME/X11 x64 已得到真实冻结包证据，建议继续采用 PyInstaller onedir + 显式 spawn。Windows 11 完整包尚未构建或运行，因此此票仍保持开放，不批准两平台实施路线已经通过。**
...
- 本轮用户已反馈“没有问题”；Windows 完整包验证方式仍待落实。此前渲染票的反馈不移用到此票。

Debian 原型已得到用户观看确认，继续采用此构建方向；获得 Windows 目标环境证据后再批准两平台路线。若 Windows 只出现可定位的资源/DLL 问题，先修正此方案并复测；没有证据要求现在另建一套 Nuitka 原型。
