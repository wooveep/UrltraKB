...
def build_workbench(window):
    ...
    actions = QHBoxLayout()
    retry = QPushButton("核对成果并重试所选任务…")
    retry.clicked.connect(window._retry_selected)
    actions.addWidget(retry)
    clear = QPushButton("清理所选任务摘要…")
    clear.clicked.connect(window._clear_selected_history)
    actions.addWidget(clear)
    tasks_layout.addLayout(actions)
